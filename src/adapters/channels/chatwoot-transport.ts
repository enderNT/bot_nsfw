...
export class ChatwootTransport implements OutboundTransport {
  constructor(private readonly settings: AppSettings["channel"]["chatwoot"]) {}
  ...
  async emit(outcome: TurnOutcome, inbound: InboundMessage): Promise<{
    ...
    const accountId = inbound.deliveryContext?.accountId ?? inbound.accountId;
    ...
    if (!this.settings.baseUrl || !this.settings.apiAccessToken) {
      throw new Error("Chatwoot outbound delivery requires CHATWOOT_BASE_URL and CHATWOOT_API_ACCESS_TOKEN");
    }

    const url = `${this.settings.baseUrl.replace(/\/$/, "")}/api/v1/accounts/${accountId}/conversations/${conversationId}/messages`;
    const response = await fetch(url, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        api_access_token: this.settings.apiAccessToken
      },
      body: JSON.stringify({
